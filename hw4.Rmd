...
For visualization, we generate two plots: one scatter plot and one histogram. For the scatter plot, the x-axis represents the range of radius from 0.1 to 1 mile, increasing by 0.1. The y-axis represents the rate of the number of dennys near la quinta within that radius. From the histogram, we can see that the increasing of radius will increase the rate. However, by drawing the line of all points, we could also see that the increasing rate, which is the tangent of the line, is slowly decreasing. 
...
df = data_frame(radius, rate.total)
ggplot(data = df, aes(x = df$radius, y = df$rate.total)) + geom_point(aes(colour = df$radius)) + geom_line(aes(colour = df$radius), size = 1) + xlab("Minimum Distance") +ylab("Rate") + ggtitle("Cumulative Distribution between Minimum Distance and Rate")
#ggplot(data = df, aes(x = df$radius)) +geom_bar(position = "identity")
